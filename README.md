...
Odoo 18 payment provider module for [Durianpay](https://durianpay.id), Indonesia's
...
- [`payment_durianpay/INSTALL.md`](./payment_durianpay/INSTALL.md) — install & usage
  guide: setting up Odoo 18 from source (manual macOS or Docker), installing the
  module, configuring the provider, and testing a sandbox payment end to end.

## Requirements

- Odoo 18 (uses the Odoo 18 `payment` module API; for Odoo 19, use the `main`
  branch instead).
...
instructions. In short: clone Odoo 18, drop this module into your custom addons path,
