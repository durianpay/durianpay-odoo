...
Targets **Odoo 18**. The webhook uses the Odoo 18 payment API:
`_process_notification_data` (find tx + apply state) with `_set_pending` / `_set_done` /
`_set_error` for the explicit state transitions. Partner phone falls back to
`res.partner.mobile`, then `res.partner.phone`.
